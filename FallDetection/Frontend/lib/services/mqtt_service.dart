...
class MqttService {
  ...
  void _onConnected() {
    ...
    tokenPublishTimer = Timer.periodic(const Duration(seconds: 45), (_) {
      _publishStreamToken();
    });
  }
  ...
  void _parseSkeletonData(Uint8Buffer payload) {
    try {
      ...
      if (bytes.length < expectedSize) {
        print(
            '⚠️ Data too small: ${bytes.length} bytes, expected $expectedSize');
        _skeletonController.add(SkeletonFrame([]));
        return;
      }
      ...
      for (int i = 0; i < numPeople; i++) {
        ...
        print(
            '  Person $personId: ${keypoints.where((k) => k.x != 0 || k.y != 0).length}/18 keypoints visible');
