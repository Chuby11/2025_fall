...
class _SkeletonVideoPlayerState extends State<SkeletonVideoPlayer> {
  ...
  void _play() {
    ...
    if (widget.totalEpochTime != null &&
        widget.numFrames != null &&
        widget.numFrames! > 0) {
      ...
      print(
          '🎬 Using skeleton timing: ${clampedInterval}ms per frame (~${calculatedFPS.toStringAsFixed(1)} FPS)');
    } else {
      // Fallback to provided frame rate
      frameDuration = Duration(milliseconds: (1000 / widget.frameRate).round());
      print(
          '🎬 Using fallback timing: ${1000 / widget.frameRate}ms per frame (${widget.frameRate} FPS)');
    }
  ...
  Widget build(BuildContext context) {
    ...
    print(
        '🎬 SkeletonVideoPlayer: ${widget.frames.length} frames, current: $currentFrameIndex');
